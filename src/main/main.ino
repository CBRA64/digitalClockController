...
uint16_t lineA_temp[6];
uint16_t lineB_temp[6];
...
/*
  ...
  --------------------------------------------------------------------------
*/
// Chain of shiftregister for lines
...
void rerouteSeg(uint16_t &src, uint16_t &dest, uint8_t srcBit, uint8_t dstBit); // Reroutes segment to follow physically rerouted segments.
void solveDriverFailures(void); // Solves driver failures that were rerouted.
void updateData(void); // Updates lines from the clock.
void updateLines(void); // Updates lines from the clock.
...
void rerouteSeg(uint16_t &src, uint16_t &dest, uint8_t srcBit, uint8_t dstBit){
  dest = (dest & ~(1 << dstBit)) | (((src >> srcBit) & 1) << dstBit);
}

void solveDriverFailures(void){
  memcpy(lineA_temp, lineA, sizeof(lineA));
  memcpy(lineB_temp, lineB, sizeof(lineB));

  // REROUTED DRIVERS SEGMENTS
...
void updateData(void){
  ...
  if (show_temperature){
    // IF TEMPERATURE WAS CORRECTLY FETCHED UPDATE LINES TO TEMPERATURE.
    ...
    // UPDATE LINE TO TEMPERATURE VALUES
    ...
    // CHANGE FROM 24HRS TO 12HRS DISPLAY AND UPDATE HOURS DATA
...
void updateLines(void){
  // Deactivates segments to pass data to registers.
  digitalWrite(CHAIN_A_N_OE, HIGH); 
  digitalWrite(CHAIN_B_N_OE, HIGH);

  // Passes data to registers.
  ...
  analogWrite(CHAIN_A_N_OE, 255-light_intensity);
  analogWrite(CHAIN_B_N_OE, 255-light_intensity);
}
